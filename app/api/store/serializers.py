...
class StoreItemReadSerializer(serializers.ModelSerializer):
    purchased = serializers.SerializerMethodField('is_purchased_by_user', read_only=True)
    ...
    def is_purchased_by_user(self, item):
        ...
        if 'request' in self.context:
            ...
            is_purchased = Transaction.objects.is_purchased(self.context.get('request').user, item)
...
class BuyTransactionSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if 'itemId' not in attrs and 'itemName' not in attrs:
            raise serializers.ValidationError('Either itemId or itemName field should be specified.')
